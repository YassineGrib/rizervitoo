-- Fix notification triggers priority constraint issue
-- This script updates existing notification trigger functions to use correct priority values
-- Run this script in Supabase SQL Editor to fix the booking cancellation error
...
DROP TRIGGER IF EXISTS trigger_notify_payment_status_changed ON public.bookings;

-- Drop existing functions
DROP FUNCTION IF EXISTS notify_booking_created();
DROP FUNCTION IF EXISTS notify_booking_status_changed();
DROP FUNCTION IF EXISTS notify_payment_status_changed();
DROP FUNCTION IF EXISTS notify_upcoming_checkins();
DROP FUNCTION IF EXISTS notify_expired_bookings();

-- Recreate function to notify booking creation
...
-- Recreate function to notify booking status change
...
-- Recreate function to notify payment status change
...
-- Recreate function to notify upcoming check-in (daily job)
...
-- Recreate function to notify expired bookings (daily job)
...
-- Success message
SELECT 'تم إصلاح محفزات الإشعارات بنجاح! يمكنك الآن إلغاء الحجوزات بدون أخطاء.' as result;